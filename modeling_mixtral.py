...
class MixtralBlockSparseTop2MLP(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        if getattr(self, "active_count_state", False):
            assert hasattr(self, "active_count")
            self.active_count += hidden_states.shape[0]
...
class MixtralSparseMoeBlock(nn.Module):
    ...
    def __init__(self, config):
        ...
        # self.expert_select_type = config.additional.expert_select_type
        # self.fix_expert_idx = config.additional.fix_expert_idx
        # self.routing_weights_source = config.additional.routing_weights_source
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        # for router mapping search
        if getattr(self, "expert_eliminate", False):
            assert getattr(self, "router_transfer", None) is not None
            try:
                assert self.router_transfer.shape[0] == self.num_experts
            except:
                import ipdb; ipdb.set_trace()
            try:
                routing_weights = routing_weights @ self.router_transfer.T.to(torch.float)
            except:
                import ipdb; ipdb.set_trace()
...
class MixtralModel(MixtralPreTrainedModel):
    ...
    def forward(
        ...
        for i, decoder_layer in enumerate(self.layers):
            ...
            if getattr(self.layers[i].block_sparse_moe, "search", False):
                print(f"Begin {i}-th decoder forward")
        ...
        #     t_end_temp = time.time()
        #     print(f"The latency of {i}-th layer is {t_end_temp - t_start_temp}")
        # t_end = time.time()
        # print(t_end-t_start)
        # import ipdb; ipdb.set_trace()
